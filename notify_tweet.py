...
class notify_tweet:
    ...
    def ask_db(self):
        ...
        # クエリを実行する
        # sql = "SELECT university,department,major,lesson,year,datetime FROM files where datetime > '2017-04-27 10:33:00'"
        if self.lastTime :
            sql = "SELECT university,department,major,lesson,year,datetime FROM files where datetime > '"+str( self.lastTime )+"'"
            print("SELECT university,department,major,lesson,year,datetime FROM files where datetime > '"+str( self.lastTime )+"'")    
        else:
            sql = "SELECT university,department,major,lesson,year,datetime FROM files"
        cur.execute(sql)
